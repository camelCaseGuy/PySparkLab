...
ratings = sc.textFile("dbfs:/FileStore/tables/ratings.csv")
movies = sc.textFile("dbfs:/FileStore/tables/movies.csv")
tags = sc.textFile("dbfs:/FileStore/tables/tags.csv")
...
# 1. 
sc.parallelize(ratingsData.countByKey().items()).sortBy(lambda x: -x[1])
moviesData.filter(lambda x: x[0]=='356').collect()

# Find the movie with the lowest count of ratings 
# 2. 
counts = sc.parallelize(ratingsData.countByKey().items()).sortBy(lambda x: x[1])
...
# 6. 
joined = sortedRatings.join(moviesData)
...
# 7. 
mathMovies = moviesData.join(tagsData).filter(lambda x: x[1][1] == "mathematics")

# What is the average ratings of movies that contain the tag 'artificial intelligence'
# 8. 
moviesData.join(tagsData).filter(lambda x: x[1][1] == "artificial intelligence").join(avgRatings)
